...
if menu == "KAMs":
    ...
    if accion_kam == "Registrar KAM":
        ...
        try:
            ...
            if "no such column: email_usuario" in str(e):
                st.error("⚠️ La base de datos necesita actualizarse. Reinicia la aplicación para aplicar las migraciones.")
                st.info("Si el problema persiste, contacta al administrador del sistema.")
                st.stop()
            else:
                raise e
        ...
        if kams:
            ...
            try:
                ...
                if "no such column" in str(e):
                    st.error("⚠️ Error de base de datos. Reinicia la aplicación.")
                    st.stop()
                else:
                    raise e
            ...
            with st.form("email_config_form"):
                email_usuario = st.text_input("Email para envío", value=kam_data[2] or "" if kam_data[2] else "", help="Email que aparecerá como remitente")
                email_password = st.text_input("Contraseña de aplicación", type="password", help="Para Gmail, genera una contraseña de aplicación")
                
                st.markdown("""
                **Instrucciones para Gmail:**
                1. Activa la verificación en 2 pasos en tu cuenta Google
                2. Ve a "Contraseñas de aplicación" en tu cuenta Google
                3. Genera una nueva contraseña de aplicación
                4. Usa esa contraseña aquí (no tu contraseña normal)
                """)
                
                submitted = st.form_submit_button("Guardar Configuración de Email")
                if submitted:
                    try:
                        run_query("UPDATE kams SET email_usuario = ?, email_password = ? WHERE id = ?", 
                                 (email_usuario, email_password, kam_id))
                        st.success("Configuración de email guardada correctamente")
                        st.rerun()
                    except sqlite3.OperationalError as e:
