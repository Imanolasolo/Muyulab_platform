                ...
                if existing_kam or existing_user:
                    ...
                    try:
                        ...
                        if email_usuario and email_password:
                            st.info(f"📧 Email configurado para envío: {email_usuario}")
                        else:
                            st.warning("⚠️ No se configuró email para envío. El KAM podrá configurarlo después o contactar al administrador.")
                            
                    except sqlite3.IntegrityError as e:
                        if "UNIQUE constraint failed: kams.email" in str(e):
                            st.error("❌ Este email ya está registrado como KAM.")
                        elif "UNIQUE constraint failed: users.email" in str(e):
                            st.error("❌ Este email ya está registrado como usuario.")
                        else:
        ...
        if kams:
            ...
            with st.form("email_config_form"):
                ...
                if email_usuario and not email_usuario.endswith('@gmail.com'):
                    st.warning("⚠️ Actualmente solo se soporta Gmail. Usa una cuenta @gmail.com")
                ...
                if submitted:
                    if email_usuario and email_password:
                        ...
                        # Probar credenciales inmediatamente
                        st.info("🔍 Probando credenciales...")
                        try:
                            import smtplib
                            with smtplib.SMTP_SSL("smtp.gmail.com", 465) as server:
                                server.login(email_usuario, email_password)
                            st.success("✅ Configuración guardada y credenciales verificadas correctamente")
                        except smtplib.SMTPAuthenticationError:
                            st.error("❌ Credenciales incorrectas. Verifica la contraseña de aplicación.")
                            st.markdown("**Solución:** Genera una nueva contraseña de aplicación en [Google](https://myaccount.google.com/apppasswords)")
                        except Exception as e:
                            st.error(f"❌ Error de conexión: {e}")
                        
                        st.rerun()
                    else:
                        st.warning("⚠️ Completa ambos campos para guardar la configuración")
        else:
                ...
                if validos_final > 0:
                    ...
                    if opciones_insercion.startswith("Insertar"):
                        if st.button("🚀 CONFIRMAR INSERCIÓN A LA BASE DE DATOS", type="primary"):
                            # Función auxiliar para insertar con commit
                            def run_insert_query_admin(query, params=()):
                                conn = sqlite3.connect(DB_PATH)
                                try:
                                    cur = conn.cursor()
                                    cur.execute(query, params)
                            ...
                            for nueva_inst in instituciones_nuevas:
                                ...
                                try:
                                    ...
                                    new_inst_id = run_query("SELECT id FROM instituciones WHERE nombre = ?", (nueva_inst,)).fetchone()[0]
